...
| 教师管理端 | 教师在课前课后使用的工作区，负责班级管理、设备管理、报表中心、资源/素材管理、个人设置 |
...
1. **管理员**
   ...
   - 通过运营管理中台创建/导入教师与学生账号、配置权限、维护基础数据

2. **教师**

   - 登录教师端后先进入教师管理端完成课前课后管理，需要授课时切换到课堂端进入课堂并发起互动/测验
...
- 场景 A：教师登录 → 选择教室 → 自动同步班级名单 → 发起选择题互动 → 实时统计 → 自动生成报表 → 导出
...
- 同一客户端双工作区：教师管理端、课堂端
- 教师管理端：班级管理、设备管理、报表中心、资源/素材管理、个人设置
...
- 主导航：班级管理、设备管理、报表中心、资源/素材管理、个人设置
...
- 主要入口：班级管理、设备管理、报表中心、资源/素材管理、个人设置、进入课堂
- 状态切换：未进入课堂时可自由切换模块；进入课堂后从顶部入口切到课堂端

**（2）班级管理页**

- 页面目标：完成班级、学生、教室关联与批量导入
- 核心区域：筛选区、班级列表区、学生明细区、批量导入/导出动作区
- 关键交互：按年级/学科/班级搜索，Excel 导入学生，查看失败原因，进入课堂
...
4. **数据范围控制**：教师仅可访问本人创建或被授权的班级、课次、学生与报表数据。
...
| 教师 | 教师管理端 | 班级管理 | ✓ | ✓ | ✓ | ✓（按授权） | Excel 导入/导出 | 仅本人或授权班级 |
...
> 使用前置：教师首次使用需创建班级/教室并导入学生信息；后续上课仅需登录后选择教室进入，系统自动同步班级信息并建立课堂互动关系。
...
1. 班级信息设置：班级名称、年级、学科、班主任、创建时间
2. 学生信息字段：姓名、性别、学号
3. 学生信息导入：支持 Excel 模板导入，模板包含学生姓名、性别、学号
4. 班级列表展示：支持按年级、学科筛选，支持搜索
5. 学生信息批量操作：批量删除、批量导出
6. 数据导出格式：Excel
