...
import { parseClipboard } from "@editor/features/clipboard";
import {
  CURSOR_TYPE,
...
import { loadFromBlob } from "../io";
import Library, { distributeLibraryItemsOnSquareGrid } from "../io/library";
import { restore, restoreElements } from "../io/restore";
import {
  dragNewElement,
...
import { mutateElement, newElementWith } from "../elements/mutateElement";
import { deepCopyElement, newFreeDrawElement } from "../elements/newElement";
import {
  hasBoundTextElement,
  isArrowElement,
  isBindingElement,
...
import { getCenter, getDistance } from "@shared/lib/gesture";
import {
...
} from "@editor/features/groups";
import History from "../state/history";
import { defaultLang, getLanguage, languages, setLanguage, t } from "@shared/i18n";
import {
  CODES,
...
import { distance2d, getGridPoint, isPathALoop } from "@shared/lib/math";
import { renderScene } from "../render/renderScene";
import { invalidateShapeForElement } from "../render/renderElement";
import {
...
import { findShapeByKey, SHAPES } from "../lib/shapes";
import {
  AppClassProperties,
  AppProps,
  AppState,
...
import { isHittingElementNotConsideringBoundingBox } from "../elements/collision";
import {
  normalizeLink,
...
} from "../elements/Hyperlink";
import { shouldShowBoundingBox } from "../elements/transformHandles";
...
class App extends React.Component<AppProps, AppState> {
  ...
  public async componentDidMount() {
    ...
    this.desktopOpenFileCleanup = requireElectronApi().onOpenFile(
      async (payload) => {
        const { file, fileHandle } = electronFilePayloadToFile(payload);
    ...
    this.updateDOMRect(() => {
      void (async () => {
        ...
        const pendingFile = await requireElectronApi().getPendingOpenFile();
        if (pendingFile) {
          const { file, fileHandle } = electronFilePayloadToFile(pendingFile);
          await this.loadFileToCanvas(file, fileHandle);
        }
      })();
    });
    ...
    if (lastPointerDownHittingLinkIcon && lastPointerUpHittingLinkIcon) {
      ...
      if (url) {
        ...
        if (!customEvent?.defaultPrevented) {
          ...
          void requireElectronApi().openExternal(normalizedUrl);
        }
      }
    }
  };
