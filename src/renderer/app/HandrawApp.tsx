...
import { ErrorDialog } from "@editor/components/ErrorDialog";
import { TopErrorBoundary } from "@editor/components/TopErrorBoundary";
import {
  EVENT,
  THEME,
  TITLE_TIMEOUT,
  VERSION_TIMEOUT,
} from "@editor/state/constants";
import {
  ExcalidrawElement,
  FileId,
...
import { Excalidraw } from "@editor/components/Excalidraw";
...
import { loadScene } from "@workspace/features/boards/loadScene";
import {
  bootstrapWorkspaceState,
  clearLibraryItems,
  getAllBoardElementsFromStorage,
  getCurrentBoardName,
  getWorkspaceDraftState,
  getLibraryItems,
  saveLibraryItems,
} from "@workspace/state/workspaceState";
import { onWorkspaceStateChanged } from "@workspace/state/workspaceEvents";
import CustomStats from "@workspace/ui/CustomStats";

import "@workspace/styles/index.scss";

import { updateStaleImageStatuses } from "@workspace/services/FileManager";
import { newElementWith } from "@editor/elements/mutateElement";
import { isInitializedImageElement } from "@editor/elements/typeChecks";
import { LocalData } from "@workspace/services/LocalData";
import { atom, Provider, useAtom } from "jotai";
import { jotaiStore } from "@editor/state/jotai";
import { useHandleLibrary } from "@editor/io/library";
import { AppMainMenu } from "@workspace/ui/AppMainMenu";
import { AppWelcomeScreen } from "@workspace/ui/AppWelcomeScreen";
import { AppFooter } from "@workspace/ui/AppFooter";
...
const initializeScene = async (): Promise<{
  ...
  const workspaceState = getWorkspaceDraftState();
  const scene = await loadScene(null, null, workspaceState);
  return { scene, isExternalScene: false };
};

const currentLangCode = detectAppLanguage();
...
const ExcalidrawWrapper = () => {
  ...
  const [theme, setTheme] = useState<Theme>(
    () => getWorkspaceDraftState().appState?.theme || THEME.LIGHT,
  );
