...
import { AppState, BinaryFileData } from "@editor/state/types";
import {
  clearAppStateForLocalState,
  getDefaultAppState,
} from "@editor/state/appState";
import { clearElementsForLocalState } from "@editor/elements";
import { DEFAULT_BOARD_NAME } from "./constants";
import { ImportedDataState } from "@editor/io/types";
import {
  getHandrawElectronApi,
  requireHandrawElectronApi,
} from "@renderer/platform/electron/api/handrawElectron";
...
export type WorkspaceBootstrapState = WorkspaceBoardState & {
  libraryItems: ImportedDataState["libraryItems"];
  settings?: WorkspaceSettings;
};

export type WorkspaceBoardSavePayload = {
  boardName: string;
  elements: readonly ExcalidrawElement[];
  appState: Partial<AppState>;
};

export type WorkspaceBoardWritePayload =
  | {
      mode: "create" | "select";
      name: string;
      elements?: readonly ExcalidrawElement[];
...
const storageCache: WorkspaceStateCache = {
  initialized: false,
...
let bootstrapPromise: Promise<WorkspaceBootstrapState> | null = null;
...
export const getElectronApi = () => getHandrawElectronApi();

export const requireElectronApi = () => requireHandrawElectronApi();
...
const applyBootstrapState = (state: WorkspaceBootstrapState) => {
  const boardList =
...
const getDefaultBootstrapState = (): WorkspaceBootstrapState => ({
  boardList: [DEFAULT_BOARD_NAME],
...
export const bootstrapWorkspaceState = async () => {
  if (!bootstrapPromise) {
    bootstrapPromise = (async () => {
      const desktop = requireElectronApi();
      const snapshot = await desktop.loadWorkspaceState();
      const nextState: WorkspaceBootstrapState = {
...
export const refreshWorkspaceState = async () => {
  ...
  return bootstrapWorkspaceState();
};

export const resetWorkspaceStateCache = () => {
  bootstrapPromise = null;
...
export const getWorkspaceDraftState = () => {
...
export const saveWorkspaceStateToStorage = async (
  elements: readonly ExcalidrawElement[],
  appState: AppState,
) => {
  const desktop = requireElectronApi();
  ...
  const nextState = await desktop.saveWorkspaceState({
...
export const getWorkspaceStateSnapshot = () => {
  return clone({
    currentBoardName: storageCache.currentBoardName,
