...
import { SceneData } from "@editor/state/types";
import { getSelectedElements } from "@editor/scene/selection";
import { ExcalidrawElement } from "@editor/elements/types";
import { setWorkspaceDraftState } from "./workspaceTestState";
...
const wrapWorkspaceTestApisWithAct = () => {
...
type TestRenderFn = (
  ...
  options?: Omit<
    RenderOptions & { workspaceStateData?: ImportedDataState },
    "queries"
  >,
) => Promise<RenderResult<typeof customQueries>>;

const renderApp: TestRenderFn = async (ui, options) => {
  if (options?.workspaceStateData) {
    await initWorkspaceState(options.workspaceStateData);
    delete options.workspaceStateData;
  }
...
const initWorkspaceState = async (data: ImportedDataState) => {
  setWorkspaceDraftState(data);
  resetWorkspaceStateCache();
