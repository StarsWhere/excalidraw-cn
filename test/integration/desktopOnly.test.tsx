...
import { installConsoleErrorFilter } from "../helpers/consoleFilters";
import {
  clearHandrawElectronApiForTests,
  getHandrawElectronApi,
  setHandrawElectronApiForTests,
...
describe("desktop-only app", () => {
  ...
  it("fails fast when the Electron preload bridge is missing", async () => {
    const originalDesktop = getHandrawElectronApi();
    clearHandrawElectronApiForTests();

    await expect(bootstrapWorkspaceState()).rejects.toThrow(
      "Handraw must run inside the Electron desktop shell",
    );

    setHandrawElectronApiForTests(originalDesktop!);
  });
